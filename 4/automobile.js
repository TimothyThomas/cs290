...
function sortArr( comparator, array ){
    /*your code here*/
    /* Using bubble sort for simplicity. Implemented using the pseudocode
     * provided on the bubble sort wikipedia entry. */
    var swapped = true;
    ...
    while (swapped) {
        swapped = false;
        for (var i = 1; i < n; i++) {
            if (comparator(new_array[i], new_array[i-1])) {
 ...
 * the first argument is larger or greater than the 2nd it returns true,
 ...
 * otherwise listed). It should be case insensitive. If two cars are of equal
 * type then the newest one by model year should be considered "greater".*/
...
var autos_by_year = sortArr(yearComparator, automobiles)
for (var i = 0; i < autos_by_year.length; i++) {
    autos_by_year[i].logMe(false);
}

console.log('\nThe cars sorted by make are:')
var autos_by_make = sortArr(makeComparator, automobiles)
for (var i = 0; i < autos_by_make.length; i++) {
    autos_by_make[i].logMe(false);
}

console.log('\nThe cars sorted by type are:')
var autos_by_type = sortArr(typeComparator, automobiles)
for (var i = 0; i < autos_by_type.length; i++) {
    autos_by_type[i].logMe(true);
}
